...
export default function BookingList() {
    ...
    try {
      ...
      if (data.bookings?.[0]) {
        console.log('🔍 First booking sample:', {
          has_user: !!data.bookings[0].user,
          user_data: data.bookings[0].user,
          user_id: data.bookings[0].user_id,
          full_name: data.bookings[0].user?.full_name
        });
      }
  ...
  // 🔧 Надёжное получение имени пользователя
  const getUserDisplayName = (booking) => {
    // Вариант 1: есть объект user с full_name
    if (booking.user?.full_name) return booking.user.full_name;
    // Вариант 2: есть объект user с username
    if (booking.user?.username) return booking.user.username;
    // Вариант 3: есть только user_id
    if (booking.user_id) return `Пользователь #${booking.user_id}`;
    // fallback
    return '—';
  };

  const getUserSecondaryInfo = (booking) => {
    if (booking.user?.username && booking.user?.full_name) {
  ...
  const getStatusBadge = (s) => {
    ...
    return (
      <span style={{ padding: '4px 10px', borderRadius: 6, fontSize: '0.8rem', fontWeight: 600, background: st.bg, color: st.color }}>
        {st.text}
      </span>
    );
  };
  ...
  if (loading && bookings.length === 0) {
    return <div style={{ padding: 40, textAlign: 'center' }}>Загрузка бронирований...</div>;
  }
  ...
  return (
    <div>
      <div style={{ marginBottom: 20, display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <select 
            ...
            style={{ padding: '8px', borderRadius: 6, border: '1px solid #cbd5e1', background: 'white' }}
          >
          ...
          <button 
            ...
            style={{ padding: '8px 16px', background: '#64748b', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer' }}
          >
            🔄 Обновить
          </button>
        </div>
      ...
      {error && (
        <div style={{ padding: '12px 16px', background: '#fee2e2', color: '#991b1b', borderRadius: 6, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
          <span>❌</span>
          <span>{error}</span>
          <button onClick={() => { setError(null); fetchBookings(pagination.page); }} style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 'bold', color: '#991b1b' }}>🔄</button>
        </div>
      )}
            ...
            {bookings.map(b => (
              <tr key={b.booking_id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                ...
                <td style={{ padding: '12px 16px' }}>
                  <div style={{ fontWeight: 500 }}>{getUserDisplayName(b)}</div>
                  {getUserSecondaryInfo(b) && (
                    <div style={{ fontSize: '0.85rem', color: '#64748b' }}>{getUserSecondaryInfo(b)}</div>
                  )}
                </td>
                <td style={{ padding: '12px 16px' }}>
                  <div>{b.computer?.serial_number || `ПК #${b.computer_id}`}</div>
                  {b.computer && (
                    <div style={{ fontSize: '0.85rem', color: '#64748b' }}>
                      Этаж {b.computer.floor}, комн. {b.computer.room}
                    </div>
                  )}
                </td>
                ...
                <td style={{ padding: '12px 16px', textAlign: 'right', whiteSpace: 'nowrap' }}>
                  {b.status === 'active' && (
                    <button 
                      ...
                      style={{ padding: '6px 10px', marginRight: 6, borderRadius: 6, border: '1px solid #cbd5e1', background: '#f8fafc', cursor: 'pointer' }}
                    >
                      ✅ Завершить
                    </button>
                  )}
                  {b.status !== 'cancelled' && b.status !== 'completed' && (
                    <button 
                      ...
                      style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid #fecaca', background: '#fff1f2', color: '#be123c', cursor: 'pointer' }}
                    >
                      🗑️ Отменить
                    </button>
                  )}
            ...
            <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {isAdmin && (
                <input 
                  ...
                  style={{ padding: '10px', borderRadius: 6, border: '1px solid #cbd5e1' }} 
                />
              )}
              <input 
                type="number" 
                ...
                onChange={e => setFormData(f => ({ ...f, computer_id: e.target.value }))} 
                required 
                style={{ padding: '10px', borderRadius: 6, border: '1px solid #cbd5e1' }} 
              />
              ...
              <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
                <button 
                  type="button" 
                  onClick={() => { setShowModal(false); setFormData({ computer_id: '', start_time: '', end_time: '', purpose: '' }); }} 
                  style={{ padding: '10px 20px', background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: 6, cursor: 'pointer' }}
                >
                  Отмена
                </button>
                <button 
